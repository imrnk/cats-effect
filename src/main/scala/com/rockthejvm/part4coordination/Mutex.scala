...
object MutexV2 {
  ...
  def createSignal[F[_]](implicit concurrent: Concurrent[F]): F[Signal[F]] =
  ...
  def createMutexWithCancellation[F[_]](
      state: Ref[F, State[F]]
  )(implicit concurrent: Concurrent[F]): MutexV2[F] =
    new MutexV2[F] {
      override def acquire =
...
object MutexPlayground extends IOApp.Simple {
  ...
  def demoLockingTasks() =
    for {
      ...
      results <-
        (1 to 10).toList.parTraverse(id => createLockingTask(id, mutex))
    } yield results
  ...
  def demoCancellingTasks() =
    for {
      mutex <- MutexV2.create[IO]
      results <-
        (1 to 10).toList.parTraverse(id => createCancellingTask(id, mutex))
    } yield results
